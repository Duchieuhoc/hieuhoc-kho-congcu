...
function tenBaiHoc({ soBai, tenBai, tiet, sgkTr, sbtTr, ma }) {
  return [
    para([run(soBai ? `BÀI ${soBai}. ${tenBai.toUpperCase()}` : tenBai.toUpperCase(), { bold: true, size: SZ_TITLE_BAI })],
...
function viDuLyThuyet(p = {}) {
  // [V12 · A1] GỠ guard chặn neo ② — Điều 18 mới: hình MẶC ĐỊNH neo phải mọi mục,
  //   chỉ căn giữa khi >8cm / Đúng-Sai / <3 dòng. hinhBenPhai chảy qua viDu như các mục khác.
  ...
  return viDu({ nhan: p.nhan || "Ví dụ", deBai: p.deBai, cacCau: p.cacCau, dapAn: p.dapAn,
                ...
                hinhBenPhai: p.hinhBenPhai, hinhBenTrai: p.hinhBenTrai });
}
...
function dangToanDayDu(p) {
  ...
  if (!p.viDuLoiGiai || (typeof p.viDuLoiGiai === "object" && !Array.isArray(p.viDuLoiGiai) && !(p.viDuLoiGiai.cacBuoc && p.viDuLoiGiai.cacBuoc.length)))
    throw new Error(`\n[LỖI KHUNG A.1] Dạng ${p.soDang ?? "?"} THIẾU lời giải mẫu (render ra "Lời giải:" rỗng).\n`
...
function bangSoLieu(duLieu, opts = {}) {
  ...
  const rows = [];
  if (tieuDe) rows.push(new TableRow({ tableHeader: true, cantSplit: true, children: tieuDe.map((o, j) => oCell(o, j, true)) }));
  hang.forEach(h => rows.push(new TableRow({ cantSplit: true, children: h.map((o, j) => oCell(o, j, false)) })));
...
// ═════════════════════════════════════════════════════════════
// [V12.1 · 29y] SƠ ĐỒ KHỐI ④ BÀI TẬP VỀ NHÀ — 2 KHỐI: A. Trắc nghiệm (Phần I/II/III con) + B. Tự luận
//   Thay sơ đồ PHẲNG cũ A/B/C/D (tieuDePhanI/II/III + tieuDeTuLuan — GIỮ để không phá bài cũ).
//   HP V12.1 Đ22.4·23.2 · Instructions AI Soạn Đại số V12.1 §④ · Phụ lục Đại số v1.7 §6.
//   Khối A/B = cấp cha (đậm + gạch chân); Phần I/II/III = cấp con (đậm, KHÔNG tiền tố A/B/C).
// ═════════════════════════════════════════════════════════════
function tieuDeKhoiTracNghiem() {
  return para([run(`A. BÀI TẬP TRẮC NGHIỆM`, { bold: true, underline: true, size: SZ_CONTENT })],
    { before: THO_RONG, after: 10, keepNext: true });
}
function tieuDeKhoiTuLuan(soBai, tongDiem) {
  return para([run(`B. BÀI TẬP TỰ LUẬN  (${soBai} bài = ${tongDiem}đ)`,
    { bold: true, underline: true, size: SZ_CONTENT })], { before: THO_RONG, after: 10, keepNext: true });
}
function tieuDePhanI_TN(soCau, tongDiem) {
  const donGia = formatDonGia(tongDiem, soCau);
  return para([run(`Phần I. Chọn đáp án  (${soCau} câu × ${donGia}đ = ${tongDiem}đ)`,
    { bold: true, size: SZ_CONTENT })], { before: THO_RONG, after: 10, keepNext: true });
}
function tieuDePhanII_TN(soCau, soMenhDe, tongDiem) {
  return para([run(`Phần II. Đúng / Sai  (${soCau} câu × ${soMenhDe} mệnh đề = ${tongDiem}đ)`,
    { bold: true, size: SZ_CONTENT })], { before: THO_RONG, after: 10, keepNext: true });
}
function tieuDePhanIII_TN(soCau, tongDiem) {
  const donGia = formatDonGia(tongDiem, soCau);
  return para([run(`Phần III. Trả lời ngắn  (${soCau} câu × ${donGia}đ = ${tongDiem}đ)`,
    { bold: true, size: SZ_CONTENT })], { before: THO_RONG, after: 10, keepNext: true });
}
...
/**
 * [28x] soGachTren — số/chữ có GẠCH TRÊN (overline), OMML <m:bar> pos=top.
 * Dùng cho số học viết theo chữ số: soGachTren("abc") = \overline{abc}; soGachTren("a1a2...an").
 * Trả qua _dmath → gắn _hhComp → lồng an toàn trong phanSo/luyThua/ngoac (không <m:oMath> lồng).
 *   para([run("Chứng minh "), H.soGachTren("abc"), run(" chia hết cho 37.")])
 */
function soGachTren(noiDung) {
  const bar = new BuilderElement({
    name: "m:bar",
    children: [
      new BuilderElement({
        name: "m:barPr",
        children: [ new BuilderElement({ name: "m:pos", attributes: { val: { key: "m:val", value: "top" } } }) ],
      }),
...
function hangHinh(items, { caoCm = 3.2, _tuLuoi = false } = {}) {
  ...
  const cells = items.map(it => {
    ...
    if (it.chuThich) kids.push(para([run(it.chuThich, { italic: true, color: C_GRAY, size: SZ_SMALL })],
      { align: AlignmentType.CENTER, before: 0, after: 60 }));
    return new TableCell({ borders: noB, verticalAlign: VerticalAlign.BOTTOM, width: { size: DXA(oCm), type: WidthType.DXA },
...
function hinhVeTextBox({ imageBuffer, rongCm = 6, tiLeGoc, chuThich }) {
  ...
  if (rongCm > 8) {
    ...
    throw new Error(
      `[LỖI HÌNH] hinhVeTextBox(): hình rộng ${rongCm}cm > 8cm → phải DÒNG RIÊNG CĂN GIỮA.\n` +
      `   HP V12.0 Điều 18 (ngoại lệ 1): hình rộng > 8cm không đủ chỗ wrap chữ → dùng H.hinhVe() căn giữa.\n` +
      `   (Vùng chữ A4 = 18.4cm; hình > 8cm để lại cột chữ ${cotChu}cm, dưới ngưỡng đọc trôi.)`
    );
  }
...
module.exports = {
  tieuDeKhoiTracNghiem, tieuDeKhoiTuLuan, tieuDePhanI_TN, tieuDePhanII_TN, tieuDePhanIII_TN,
  kiemMay,
  viDuLyThuyet,
  ...
  luyThua, luyThuaUnicode, phanSo, ngoac, canBac, soGachTren, chiSoDuoi, triTuyetDoi, kyHieuGoc, hinhVe, hangHinh, luoiHinh, hinhVeTextBox, paraCoHinhPhai, hePhuongTrinh, paraHePhuongTrinh,
